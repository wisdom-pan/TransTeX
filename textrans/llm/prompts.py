...
6. 保留原文首尾的空格与换行结构。
7. 只要文本里含有英文自然语言词句,就必须翻译成{target_lang}——**即使它是句子片段、以小写字母开头、看起来不完整、或被引用/公式从中间截断**(它是长句被 LaTeX 命令切开的一部分,你只需翻译这一片,不要因为"不完整"而原样返回)。仅当整段确实是纯符号、纯数字、纯 LaTeX 代码或已经是{target_lang}时,才原样返回。
